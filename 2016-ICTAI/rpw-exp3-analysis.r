...
# for that data.
getObjData = function() {
  rawData.CHC  = read.table('EXP3-real-CHC-x50/bst.test.all',   header=F)
  rawData.PHCI = read.table('EXP3-real-PHC-I-x50/bst.test.all', header=F)
  rawData.PHCP = read.table('EXP3-real-PHC-P-x50/bst.test.all', header=F)
  
  return ( rbind(convertObjData(rawData.CHC, "P-CHC"),
                 convertObjData(rawData.PHCI, "P-PHC-I"),
                 convertObjData(rawData.PHCP, "P-PHC-P") ) )
}
...
getDispData = function() {
  rawData.CHC  = read.table('EXP3-real-CHC-x50/test_xy_difference_merged',   header=F)
  rawData.PHCI = read.table('EXP3-real-PHC-I-x50/test_xy_difference_merged', header=F)
  rawData.PHCP = read.table('EXP3-real-PHC-P-x50/test_xy_difference_merged', header=F)
